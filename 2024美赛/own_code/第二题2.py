...
np.random.seed(43)
a = stats.norm.rvs(525, 350, 100)  # 随机生态位
print(a)
...
for k in range(100):
    ...
    for t in range(1, total_t):
        m_1[t] = m_1[t - 1] + 0.2 * m_1[t - 1] * (1 - (m_1[t - 1] + 0.1 * f_1[t - 1])/a[k])
        f_1[t] = f_1[t - 1] + 0.2 * f_1[t - 1] * (1 - (0.3 * m_1[t - 1] + f_1[t - 1])/a[k])
        m_2[t] = m_2[t - 1] + 0.4 * m_2[t - 1] * (1 - (m_2[t - 1] + f_2[t - 1]) / a[k])
        f_2[t] = f_2[t - 1] + 0.5 * f_2[t - 1] * (1 - (m_2[t - 1] + f_2[t - 1]) / a[k])
        if m_1[t]< 0:
            m_1[t] = 0
        if f_1[t] < 0:
            f_1[t] = 0
