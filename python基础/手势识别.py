...
def findhands(img, hands, draw):
    ...
    if results.multi_hand_landmarks:
        for handLms in results.multi_hand_landmarks:
            draw.draw_landmarks(
                img, handLms, mp.solutions.hands.HAND_CONNECTIONS, handlmsstyle, handconstyle)
...
def detectednumber(hand_landmarks, img):
    ...
    dist_thumb2index = math.sqrt(
        (thumb_tip_x - index_finger_tip_x)**2 + (thumb_tip_y - index_finger_tip_y)**2)
    dist_thumb2middle = math.sqrt(
        (thumb_tip_x - middle_finger_tip_x)**2 + (thumb_tip_y - middle_finger_tip_y)**2)
    dist_thumb2ring = math.sqrt(
        (thumb_tip_x - ring_finger_tip_x)**2 + (thumb_tip_y - ring_finger_tip_y)**2)
    dist_thumb2pinky = math.sqrt(
        (thumb_tip_x - pinky_tip_x)**2 + (thumb_tip_y - pinky_tip_y)**2)
    dist_thumb2mcp = math.sqrt(
        (thumb_tip_x - pinky_mcp_x)**2 + (thumb_tip_y - pinky_mcp_y)**2)
    print(dist_thumb2index, dist_thumb2middle,
          dist_thumb2ring, dist_thumb2pinky, dist_thumb2mcp)
    ...
    if ret:
        ...
        if hand_landmarks:
            ...
            if detected_number >= 0:
                cv2.putText(img, str(detected_number), (300, 300),
                            cv2.FONT_HERSHEY_TRIPLEX, 10, (0, 255, 0))
